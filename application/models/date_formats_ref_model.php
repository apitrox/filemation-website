...
class Date_formats_ref_model extends CI_Model {
	...
	//  This method will retrieve and return all date format ref records
	//  @Return:	result array
	public function GetAllDateFormats()
	{
		...
		$this->db->order_by('Date_Format_Order', 'ASC');
		$result = $this->db->get('date_formats_ref');
		$sql_stmt = $this->db->last_query() ;
		$error = ( is_null($result) || ($result == FALSE) );   $err_msg = "Retrieving all date format ref records failed. \$result returned false. \nSQL Statement: \n$sql_stmt";   $notify = FALSE;   $severity = "ERROR";   $stop = TRUE;   $display = FALSE;    /* -- Do Not Edit beyond this point on this line -- */   if($error){$err_log_id = $this->Error_log_model->NewErrorLogEntry3(TRUE, $notify, $severity, $err_msg, $display, __METHOD__, __LINE__);}  if($stop && $error){return array('Result' => FALSE, 'Error_Log_Id' => $err_log_id, 'Result_Message' => $err_msg . "\nError_Log_Id:  $err_log_id");}
		...
		if( $result->num_rows() > 0 )
		{
			...
			{
				$date_entry = array();
				$date_entry['Date_Format_Id'] = $row->Date_Format_Id;
				$date_entry['Date_Format_Order'] = $row->Date_Format_Order;
				$date_entry['Date_Format_Desc'] = $row->Date_Format_Desc;
				$date_entry['Php_Date_Format_Str'] = $row->Php_Date_Format_Str;
				$date_entry['JavaScript_Date_Format_Str'] = $row->JavaScript_Date_Format_Str;
